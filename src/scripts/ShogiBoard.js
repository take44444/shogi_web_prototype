...
const BOARD_STATE = {
  NOTHING: 0, OUTE: 1, TUMI: 2, SENNICHITE: 3,
};
...
class ShogiBoard {
  ...
  constructor() {
    ...
    for (let turn = 0; turn <= 1; turn++) {
      ...
      this.tegoma_[turn]['FU'] = {
        koma: new Fu(turn == SENTE), num: 0,
      };
      this.tegoma_[turn]['KY'] = {
        koma: new Ky(turn == SENTE), num: 0,
      };
      this.tegoma_[turn]['KE'] = {
        koma: new Ke(turn == SENTE), num: 0,
      };
      this.tegoma_[turn]['GI'] = {
        koma: new Gi(turn == SENTE), num: 0,
      };
      this.tegoma_[turn]['KI'] = {
        koma: new Ki(turn == SENTE), num: 0,
      };
      this.tegoma_[turn]['HI'] = {
        koma: new Hi(turn == SENTE), num: 0,
      };
      this.tegoma_[turn]['KA'] = {
        koma: new Ka(turn == SENTE), num: 0,
      };
    }
  ...
  sandbox() {
    ...
    for (let turn = 0; turn <= 1; turn++) {
      for (const koma in this.tegoma_[turn]) {
        if (true) {
  ...
  move(from, to, koma) {
    ...
    if (!this.board_[to.x][to.y].isEmpty) {
      if (this.board_[to.x][to.y].isNari) {
        /** 取る駒が，成っている駒の場合 */
        this.tegoma_[+this.turn_][this.board_[to.x][to.y].createNarazu().symbol]
            .num++;
      } else {
        /** 取る駒が，成ってない駒の場合 */
   ...
   */
  isTumi(checkTurn) {
    const target = this.ou_[+!checkTurn];
    const ou = this.board_[target.x][target.y];
    for (const path of ou.pathGen(target.x, target.y, this.board_)) {
      /** 利きがない移動先がある場合は詰みではない */
      const tmp = this.board_[path.x][path.y];
    ...
    const guardIterList = this.searchKiller(target, checkTurn, MODE.ITERATOR);
    /** 両王手なら詰み */
    if (guardIterList.length >= 2) {
      return true;
    }
    /** 王手を回避することができるか */
    const tegoma = this.tegoma_[+!checkTurn];
    for (const guardPath of guardIterList[0]) {
      /** 手駒を用いて防ぐことができるか */
      if (this.board_[guardPath.x][guardPath.y].isEmpty) {
        for (const key in tegoma) {
          if (tegoma[key].num > 0 &&
      ...
      for (const guardPoint of
        this.searchKiller(guardPath, !checkTurn, MODE.POINT)) {
        const guard = this.board_[guardPoint.x][guardPoint.y];
  ...
  searchKiller(target, checkTurn, mode=MODE.BOOL) {
    ...
    for (const path of
      new Ke(!checkTurn).pathGen(target.x, target.y, this.board_)) {
    ...
    /** 隣接マスからの利きを調べる */
    for (const path of
      new Ou(!checkTurn).pathGen(target.x, target.y, this.board_)) {
      const killer = this.board_[path.x][path.y];
      for (const killerPath of killer.pathGen(path.x, path.y, this.board_)) {
    ...
    if (checkTurn) {
      ret = accumulate(ret, this.searchKillerSub(target, ['HI', 'RY', 'KY'],
          true,
          (x, i) => {
            return x;
          },
  ...
  searchKillerSub(target, checkList, checkTurn, updateX, updateY,
    ...
    search:
    for (let dif = 1; ; dif++) {
      const x = updateX(target.x, dif);
      const y = updateY(target.y, dif);
      const killer = this.board_[x][y];
